...
it('should render title', () => {
	render(<VoipPopupHeader>voice call header title</VoipPopupHeader>, { wrapper: appRoot.build() });

	expect(screen.getByText('voice call header title')).toBeInTheDocument();
});

it('should not render close button when onClose is not provided', () => {
	render(<VoipPopupHeader />, { wrapper: appRoot.build() });

	expect(screen.queryByRole('button', { name: 'Close' })).not.toBeInTheDocument();
});

it('should render close button when onClose is provided', () => {
	render(<VoipPopupHeader onClose={jest.fn()} />, { wrapper: appRoot.build() });
...
it('should call onClose when close button is clicked', () => {
	...
	render(<VoipPopupHeader onClose={closeFn} />, { wrapper: appRoot.build() });
...
it('should not render settings button when hideSettings is true', () => {
	render(<VoipPopupHeader hideSettings>text</VoipPopupHeader>, { wrapper: appRoot.build() });
